...
3. 佐證分三種，權重不同：
   - 「法規原文（本地法規庫）」是官方原文全文，可作為權威依據直接引用。
   - 「官方法規庫檢索結果（国家税务总局）」是官方來源，其標題、文號、
     成文日期與時效性可直接採信並引用；但僅提供摘要而非全文，
     不得憑摘要臆測未列出的條文內容。
...
5. confidence 欄位反映你對整體分析的信心度：
   - 0.9-1.0: 條文明確，且有法規原文（本地法規庫）全文交叉驗證
   - 0.85-0.95: 條文明確，且有官方法規庫檢索結果佐證（官方來源，僅摘要）
   - 0.7-0.9: 條文明確，但僅有網路搜尋結果佐證，或佐證僅部分吻合
